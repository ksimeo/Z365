...
<head>
    ...
    <link type="text/javascript" rel="../../../resources/js/admins.js">
    ...
    <form:form class="form-horizontal" method="POST" modelAttribute="orderForm" action="${userActionUrl}">
                ...
                <select id="prodName" name="product" size="1" onchange="checkParams()">
                        ...
                        <c:forEach items="${prods}" var="item">
                            <c:if test="${item.actual}">
                                <option name="type" value="${item.id}" ${item.id == prodType ? 'selected="selected"' : ''}/>${item.name}</option>
                            </c:if>
                        </c:forEach>
                    </select>
                </label>
        ...
        <div class="form-group">
            <div class="col-sm-offset-2 col-sm-10">
                ...
                <button type="submit" id="submit" class="btn-lg btn-primary pull-right">
